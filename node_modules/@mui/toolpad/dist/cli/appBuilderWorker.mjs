...
var UPGRADE_URL = "https://mui.com/toolpad/getting-started/roadmap/#paid-plan";
      ...
      if (/\/resources\//.test(id)) {
        ...
        for (let i = 0; i < lines.length; i += 1) {
          ...
          if (/\s*export\b/.test(line)) {
            const match2 = line.match(/\s*export\s+async\s+function\s+([a-zA-Z0-9]+)\b/);
            if (match2) {
...
import { errorFrom as errorFrom7 } from "@mui/toolpad-utils/errors";
...
var pageSchema = toolpadObjectSchema(
  ...
  z.object({
    ...
    unstable_codeFile: z.coerce.boolean().optional().describe("The content of the page as JSX. Experimental, do not use!."),
    display: z.union([
      z.literal("standalone").describe("Hide the Toolpad chrome and just display the content of the page"),
...
import { ensureSuffix, indent as indent2 } from "@mui/toolpad-utils/strings";
...
import { TOOLPAD_DATA_PROVIDER_MARKER } from "@mui/toolpad-core/server";
import * as z2 from "zod";
import { fromZodError } from "zod-validation-error";
import * as crypto from "crypto";
async function loadExports(filePath) {
  const fullPath = path5.resolve(filePath);
...
// src/server/FunctionsManager.ts
var currentDirectory2 = url2.fileURLToPath(new URL(".", String(import.meta.url)));
...
var FunctionsManager = class {
  ...
  getResourcesFolder() {
    return path6.join(this.project.getRoot(), "./resources");
  }
  getFunctionsFile() {
    return path6.join(this.getResourcesFolder(), "./functions.ts");
  }
  async getFunctionFilePath(fileName) {
    const resourcesFolder = this.getResourcesFolder();
    return path6.join(resourcesFolder, fileName);
  }
  getFunctionResourcesPattern() {
    return path6.join(this.getResourcesFolder(), "*.ts");
  }
  async migrateLegacy() {
    const legacyQueriesFile = path6.resolve(this.project.getRoot(), "queries.ts");
    if (await fileExists(legacyQueriesFile)) {
      const functionsFile = this.getFunctionsFile();
      await fs3.mkdir(path6.dirname(functionsFile), { recursive: true });
  ...
  async getFunctionFiles() {
    ...
    return paths.map((fullPath) => path6.relative(this.project.getRoot(), fullPath));
  }
  ...
  getFunctionsOutputFolder() {
    return path6.resolve(this.project.getOutputFolder(), "functions");
  }
  getIntrospectJsonPath() {
    return path6.resolve(this.getFunctionsOutputFolder(), "introspect.json");
  }
  async extractTypes() {
    if (!this.extractTypesWorker) {
      this.extractTypesWorker = new Piscina({
        filename: path6.resolve(currentDirectory2, "../cli/functionsTypesWorker.mjs")
      });
    }
  ...
  async getBuiltOutputFilePath(fileName) {
    const resourcesFolder = this.getResourcesFolder();
    const fullPath = path6.resolve(resourcesFolder, fileName);
    const entryPoint = path6.relative(this.project.getRoot(), fullPath);
    const buildErrors = this.getBuildErrorsForFile(entryPoint);
    if (buildErrors.length > 0) {
      throw formatError(buildErrors[0]);
    }
    const outputFilePath = path6.resolve(
      this.getFunctionsOutputFolder(),
      `${path6.basename(fileName, ".ts")}.mjs`
    );
  ...
  async createFunctionFile(name) {
    const filePath = path6.resolve(this.getResourcesFolder(), ensureSuffix(name, ".ts"));
...
import { fileExists as fileExists2 } from "@mui/toolpad-utils/fs";
...
async function detectPackageManager(root) {
  const [hasYarnLock, hasPackageLock, hasPnpmLock] = await Promise.all([
    fileExists2(path7.resolve(root, "./yarn.lock")),
    fileExists2(path7.resolve(root, "./package-lock.lock")),
    fileExists2(path7.resolve(root, "./pnpm-lock.lock"))
  ]);
  if (hasYarnLock) {
    return "yarn";
  }
...
import { createServerJsRuntime as createServerJsRuntime2 } from "@mui/toolpad-core/jsServerRuntime";
import express from "express";
import cors from "cors";
import invariant2 from "invariant";
import { errorFrom as errorFrom6, serializeError as serializeError3 } from "@mui/toolpad-utils/errors";
...
import { serializeError, errorFrom as errorFrom2 } from "@mui/toolpad-utils/errors";
...
import { errorFrom as errorFrom5, serializeError as serializeError2 } from "@mui/toolpad-utils/errors";
...
import { ensureSuffix as ensureSuffix2 } from "@mui/toolpad-utils/strings";
...
function getDefaultUrl(config2, connection) {
  const baseUrl = connection?.baseUrl;
  return baseUrl ? "" : new URL("/static/movies.json", config2.externalUrl || window.location.href).href;
}

// src/toolpadDataSources/rest/server.ts
async function loadEnvFile(project) {
...
// src/toolpadDataSources/googleSheets/server.ts
function createOAuthClient(externalUrl) {
...
function createDatasource2(project) {
  ...
  createDataHandler() {
    ...
    router.post(
      ...
      asyncHandler(async (req, res) => {
        ...
        invariant2(typeof pageName === "string", "pageName url param required");
        invariant2(typeof queryName === "string", "queryName url variable required");
...
var currentDirectory3 = url3.fileURLToPath(new URL(".", String(import.meta.url)));
var pkgJsonContent2 = fs4.readFileSync(path8.resolve(currentDirectory3, "../../package.json"), {
  encoding: "utf-8"
});
...
function getThemeFile(root) {
  return path9.join(root, "./theme.yml");
}
function getApplicationFile(root) {
  return path9.join(root, "./application.yml");
}
function getComponentsFolder(root) {
  return path9.join(root, "./components");
}
function getPagesFolder(root) {
  return path9.join(root, "./pages");
}
function getPageFolder(root, name) {
  const pagesFolder = getPagesFolder(root);
  const pageFolder = path9.resolve(pagesFolder, name);
  return pageFolder;
}
function getPageFile(root, name) {
  const pageFolder = getPageFolder(root, name);
  const pageFileName = path9.resolve(pageFolder, "page.yml");
  return pageFileName;
}
function getComponentFilePath(componentsFolder, componentName) {
  return path9.join(componentsFolder, `${componentName}.tsx`);
}
function getOutputFolder(root) {
  return path9.join(root, ".generated");
}
function getAppOutputFolder(root) {
  return path9.join(getOutputFolder(root), "app");
}
  ...
  const resultEntries = await Promise.all(
    entries.map(async (entry) => {
      if (entry.isDirectory()) {
        ...
        const pageDirEntries = new Set(await fs5.readdir(path9.resolve(pagesFolder, pageName)));
        if (pageDirEntries.has("page.yml")) {
          const ymlFilePath = path9.resolve(pagesFolder, pageName, "./page.yml");
...
async function initGitignore(root) {
  const generatedGitignorePath = path9.resolve(root, ".gitignore");
...
function expandChildren(children, dom) {
  return children.sort((child1, child2) => {
    invariant3(child1.parentIndex && child2.parentIndex, "Nodes are not children of another node");
...
function createPageDomFromPageFile(pageName, pageFile) {
  ...
  let fragment = appDom3.createFragment("page", {
    ...
    attributes: {
      ...
      codeFile: pageFileSpec.unstable_codeFile || void 0,
...
function getDomFilePatterns(root) {
  return [
    path9.resolve(root, "./pages/*/page.yml"),
    path9.resolve(root, "./theme.yml"),
    path9.resolve(root, "./application.yml")
  ];
}
...
function detectPaidFeatures(application) {
  ...
  const paidFeatures = [
    ...
    hasAzureActiveDirectory ? { id: "azure-ad", label: "Azure AD authentication provider" } : void 0
  ].filter(Boolean);
...
var ToolpadProject = class {
  ...
  initWatcher() {
    ...
    chokidar3.watch(
      [path9.resolve(this.root, "./components"), path9.resolve(this.root, "./components/*.*")],
      watchOptions
    ...
    chokidar3.watch(
      [path9.resolve(this.root, "./pages"), path9.resolve(this.root, "./pages/*/page.*")],
      watchOptions
  ...
  getBuildInfoFile() {
    return path9.resolve(this.getOutputFolder(), "buildInfo.json");
  }
  ...
  async buildComponentsManifest() {
    ...
    const result = entries.map((entry) => {
      if (entry.isFile()) {
        ...
        const filePath = path9.resolve(componentsFolder, fileName);
  ...
  async openCodeEditor(fileName, fileType) {
    ...
    const fullResolvedPath = path9.resolve(root, resolvedPath);
  ...
  async getRuntimeConfig() {
    invariant3(this.options.externalUrl, "External URL is not set");
    return {
...
async function buildPagesManifest(root) {
  ...
  const pages = (await Promise.all(
    pageDirs.map(async (page) => {
      if (page.isDirectory()) {
        const pagePath = path9.resolve(pagesFolder, page.name);
        const title = guessTitle(page.name);
        const extensions = [".tsx", ".jsx"];
        for (const extension of extensions) {
          const pageFilePath2 = path9.resolve(pagePath, `page${extension}`);
        ...
        const pageFilePath = path9.resolve(pagePath, "page.yml");
...
async function main() {
  ...
  invariant4(!!process.env.TOOLPAD_PROJECT_DIR, "A project root must be defined");
  invariant4(!!process.env.TOOLPAD_BASE, "A base path must be defined");
