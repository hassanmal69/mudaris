...
var APP_URL_WINDOW_PROPERTY = "__TOOLPAD_APP_URL__";
...
// src/server/toolpadAppServer.ts
function postProcessHtml(html, { config: config2, initialState }) {
  const serializedConfig = serializeJavascript(config2, { ignoreFunction: true });
...
import { errorFrom as errorFrom8 } from "@mui/toolpad-utils/errors";
import { filterValues, hasOwnProperty as hasOwnProperty2, mapValues } from "@mui/toolpad-utils/collections";
...
function toolpadObjectSchema(kind, spec) {
  return z2.object({
    ...
    kind: z2.literal(kind).describe(`Describes the nature of this Toolpad "${kind}" object.`),
        ...
        z2.object({
          ...
          roles: z2.array(
            z2.object({
              ...
              target: z2.string().describe("Toolpad role to be mapped to.")
            })
...
var pageSchema = toolpadObjectSchema(
  ...
  z2.object({
    ...
    unstable_codeFile: z2.coerce.boolean().optional().describe("The content of the page as JSX. Experimental, do not use!."),
    display: z2.union([
      z2.literal("standalone").describe("Hide the Toolpad chrome and just display the content of the page"),
      z2.literal("shell").describe("Show Toolpad header and navigation.")
    ]).optional().describe(
...
// src/server/FunctionsManager.ts
var currentDirectory = url.fileURLToPath(new URL(".", String(import.meta.url)));
...
import { withContext as withContext2, createServerContext as createServerContext2 } from "@mui/toolpad-core/serverRuntime";
import { createServerJsRuntime as createServerJsRuntime2 } from "@mui/toolpad-core/jsServerRuntime";
import express4 from "express";
import cors from "cors";
import invariant2 from "invariant";
import { errorFrom as errorFrom7, serializeError as serializeError4 } from "@mui/toolpad-utils/errors";
...
import { serializeError as serializeError2, errorFrom as errorFrom3 } from "@mui/toolpad-utils/errors";
...
var currentDirectory3 = url3.fileURLToPath(new URL(".", import.meta.url));
...
async function createEditorHandler(appUrl, { toolpadDevMode = false }) {
  ...
  if (toolpadDevMode) {
    console.log(`${chalk5.blue("info")}  - Running Toolpad editor in dev mode`);
    const vite = await import("vite");
    viteApp = await vite.createServer({
      configFile: path8.resolve(currentDirectory3, "../../src/toolpad/vite.config.mts"),
      root: path8.resolve(currentDirectory3, "../../src/toolpad"),
    ...
    router.use(
      "/",
      express5.static(path8.resolve(currentDirectory3, "../../dist/editor"), { index: false }),
      asyncHandler(async (req, res) => {
        const htmlFilePath = path8.resolve(currentDirectory3, "../../dist/editor/index.html");
        let html = await fs6.readFile(htmlFilePath, { encoding: "utf-8" });
...
async function runEditor(appUrl, options = {}) {
  let appRootUrl;
  try {
    appRootUrl = await fetchAppUrl(appUrl);
  } catch (err) {
    console.error(
      `${chalk5.red("error")} - No Toolpad application found running under ${chalk5.cyan(appUrl)}
  ...
  console.log(`${chalk5.blue("info")}  - starting Toolpad editor...`);
  const app = express5();
  const editorBasename = "/_toolpad";
  ...
  console.log(
    `${chalk5.green("ready")} - Toolpad editor ready on ${chalk5.cyan(
...
async function runApp({
  ...
  console.log(
    `${chalk5.green("ready")} - toolpad project ${chalk5.cyan(projectDir)} ready on ${chalk5.cyan(
      toolpadUrl
    )}`
  );
  if (dev) {
...
// cli/index.ts
var currentDirectory4 = url4.fileURLToPath(new URL(".", import.meta.url));
...
async function devCommand(args) {
  console.log(`${chalk6.blue("info")}  - starting Toolpad application in dev mode...`);
...
async function buildCommand({ dir, base }) {
  const projectDir = path9.resolve(process.cwd(), dir);
  console.log(`${chalk6.blue("info")}  - building Toolpad application...`);
...
async function startCommand(args) {
  console.log(`${chalk6.blue("info")}  - Starting Toolpad application...`);
...
async function cli(argv) {
  ...
  const parsedArgs = yargs(argv).scriptName("toolpad").command(
    ["$0 [dir]", "dev [dir]"],
